...
for level in levels:
    ...
    summary_industry['Aggregation after Footprint Calculation'] = pickle.load(open(outputs_filepath + 'summary_' + level + '_agg_after.p', 'rb'))
    summary_industry['Aggregation before Footprint Calculation'] = pickle.load(open(outputs_filepath + 'summary_' + level + '_agg_before.p', 'rb'))
    ...
    for item in ['Total', 'Imports']:
        ...
        fig.tight_layout()
        plt.savefig(plot_filepath + 'pointplot_CO2_global_by_sector_GHG_agg&og_' + item + '_' + level + '.png', dpi=200, bbox_inches='tight')
        plt.show()
    ...
    fig.tight_layout()
    plt.savefig(plot_filepath + 'scatterplot_CO2_global_by_sector_GHG_agg&og_all_' + level + '.png', dpi=200, bbox_inches='tight')
    plt.show()
