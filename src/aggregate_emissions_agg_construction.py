...
# Load data
co2_gloria = {year: pd.read_csv('O:/ESCoE_Project/data/Emissions/Gloria/Gloria_CO2_' + str(year) + '_agg_construction.csv', index_col=[0,1], header=[0, 1]) for year in years}# should be 2010
...
co2_exio = pickle.load(open(emissions_filepath + 'Exiobase/Exiobase_emissions_agg_construction.p', 'rb'))
...
for year in years:
    ...
    gloria_sectors['Construction'] = 'Construction' 
    ...
    exio_sectors['Construction'] = 'Construction' 
...
pickle.dump(co2_all, open(emissions_filepath + 'Emissions_aggregated_all_agg_construction.p', 'wb'))
