...
for agg_var in agg_vars:   
    ...
    rmse_pct = pickle.load(open(outputs_filepath + 'rmse_pct_country_' + agg_var + '.p', 'rb'))
    direction = pickle.load(open(outputs_filepath + 'direction_annual_country_' + agg_var + '.p', 'rb'))
    reg_results = pickle.load(open(outputs_filepath + 'regression_country_' + agg_var + '.p', 'rb'))
    
    reg_validation = pickle.load(open(outputs_filepath + 'regression_validation_' + agg_var + '.p', 'rb'))
    reg_fit = {item:pd.read_csv(outputs_filepath + 'regression_country_linieraty_AIC_' + item + '.csv') for item in ['Total', 'Imports']}
    
    country_order = pickle.load(open(outputs_filepath + 'country_order_' + agg_var + '.p', 'rb'))[order_var]
    ...
    fig.tight_layout()
    plt.savefig(plot_filepath + 'scatterplot_overview_bycountry_GHG_' + order_var + '_' + agg_var + '.png', dpi=200, bbox_inches='tight')
    plt.show()
