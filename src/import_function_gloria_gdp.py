...
def get_metadata_indices(mrio_filepath, lookup_filepath, labels_fname, lookup_fname):
    ...
    # get value added categories
    va_cats = pd.DataFrame(labels['Sequential region-sector labels']['Sequential_valueAdded_labels'].dropna(how='all', axis=0)); va_cats.columns = ['label']
    country, country_code, remainder=split_country_and_code(va_cats['label'], valid_country_codes)        
    va_cats['country_code'] = country_code
    va_cats['va'] = remainder
    ...
    v_cols = pd.MultiIndex.from_arrays([va_cats['country_code'], va_cats['va']]) # labels for VA dataframe
    ...
    return z_idx, industry_idx, product_idx, iix,pix, y_cols, v_cols, sat_rows
...
def read_data_new(v_filepath, iix, pix, industry_idx, product_idx, v_cols):
    ...
    V = pd.read_csv(v_filepath, header=None, index_col=None)#, skiprows = lambda x: x not in iix)
    V.columns=v_cols
